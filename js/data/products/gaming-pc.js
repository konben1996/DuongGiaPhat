window.DuongGiaStoreProducts = window.DuongGiaStoreProducts || {};
window.DuongGiaStoreProducts["gaming-pc"] = {
  products: [
    {
      id: "pcap-apollo",
      name: "Máy tính chơi game PCAP Apollo",
      price: 13500000,
      oldPrice: 15500000,
      discount: 13,
      category: "gaming-pc",
      label: "PC Gaming",
      image:
        "assets/products/gaming-pc/pcap-apollo/pcap-apollo.jpg",
      stock: "Còn hàng",
      brand: "ASUS",
      summary:
        "Bộ PC gaming tầm trung với hiệu năng ổn định cho eSports, làm việc đa nhiệm và giải trí hằng ngày.",
      specs: [
        "CPU Intel Core i5 thế hệ mới",
        "RAM 16GB DDR4",
        "SSD NVMe 512GB tốc độ cao",
        "Card đồ họa RTX tối ưu gaming",
      ],
      tags: ["bestSeller", "discount"],
    },
    {
      id: "pcap-styx",
      name: "Máy tính đa tác vụ PCAP Styx",
      price: 13500000,
      oldPrice: 15500000,
      discount: 13,
      category: "gaming-pc",
      label: "PC Đa nhiệm",
      image:
        "assets/products/gaming-pc/pcap-styx/pcap-styx.jpg",
      stock: "Còn hàng",
      brand: "MSI",
      summary:
        "Dòng PC tối ưu làm việc đa nhiệm, học tập, văn phòng nặng và xử lý đồ họa cơ bản.",
      specs: ["CPU Intel Core i5", "RAM 16GB", "SSD 512GB", "Thiết kế gọn gàng"],
      tags: ["bestSeller", "discount"],
    },
    {
      id: "pcap-iris",
      name: "Máy tính đa tác vụ PCAP Iris",
      price: 23500000,
      oldPrice: 25500000,
      discount: 8,
      category: "gaming-pc",
      label: "PC Đa nhiệm",
      image:
        "assets/products/gaming-pc/pcap-iris/pcap-iris.jpg",
      stock: "Còn hàng",
      brand: "INTEL",
      summary:
        "Cấu hình mạnh cho làm việc nặng, chỉnh sửa ảnh, dựng video nhẹ và chiến game phổ biến.",
      specs: ["Core i7", "RAM 32GB", "SSD 1TB", "Card đồ họa rời"],
      tags: ["bestSeller", "discount", "newArrival"],
    },
    {
      id: "pcap-poseidon",
      name: "Máy tính chơi game PCAP Poseidon",
      price: 23500000,
      oldPrice: 25500000,
      discount: 8,
      category: "gaming-pc",
      label: "PC Gaming",
      image:
        "assets/products/gaming-pc/pcap-poseidon/pcap-poseidon.jpg",
      stock: "Sẵn cấu hình",
      brand: "NVIDIA",
      summary: "PC gaming hiệu năng cao cho AAA, streaming, thiết kế 2D/3D và nâng cấp linh hoạt.",
      specs: ["Ryzen/Intel tùy chọn", "RAM 32GB", "RTX series", "Tản nhiệt tối ưu"],
      tags: ["discount", "bestSeller"],
    },
    {
      id: "pcap-jupiter",
      name: "Máy tính chơi game PCAP Jupiter",
      price: 20500000,
      oldPrice: 22500000,
      discount: 9,
      category: "gaming-pc",
      label: "PC Gaming",
      image:
        "assets/products/gaming-pc/pcap-jupiter/pcap-jupiter.jpg",
      stock: "Còn hàng",
      brand: "GIGABYTE",
      summary: "Dòng PC dành cho game thủ cần cấu hình mạnh, ổn định và nâng cấp lâu dài.",
      specs: ["CPU mạnh", "RAM 16GB", "SSD NVMe", "Case kính cường lực"],
      tags: ["discount", "newArrival"],
    },
    {
      id: "asus-strix-ultra",
      name: "Máy tính chơi game PCAP ASUS STRIX ULTRA 1",
      price: 33500000,
      oldPrice: 35500000,
      discount: 6,
      category: "gaming-pc",
      label: "PC Gaming",
      image:
        "assets/products/gaming-pc/asus-strix-ultra/asus-strix-ultra.jpg",
      stock: "Sẵn cấu hình",
      brand: "ASUS",
      summary: "Bộ máy gaming cao cấp định hướng eSports và AAA với ngoại hình nổi bật.",
      specs: ["Main ASUS Strix", "RTX series", "RGB đồng bộ", "Tản khí cao cấp"],
      tags: ["newArrival"],
    },
  ],
  productGalleries: {
    "pcap-apollo": [
      "assets/products/gaming-pc/pcap-apollo/pcap-apollo.jpg",
      "assets/products/gaming-pc/pcap-apollo/pcap-apollo.jpg",
    ],
    "pcap-styx": [
      "assets/products/gaming-pc/pcap-styx/pcap-styx.jpg",
      "assets/products/gaming-pc/pcap-styx/pcap-styx.jpg",
    ],
    "pcap-iris": [
      "assets/products/gaming-pc/pcap-iris/pcap-iris.jpg",
      "assets/products/gaming-pc/pcap-iris/pcap-iris.jpg",
    ],
    "pcap-poseidon": [
      "assets/products/gaming-pc/pcap-poseidon/pcap-poseidon.jpg",
      "assets/products/gaming-pc/pcap-poseidon/pcap-poseidon.jpg",
    ],
    "pcap-jupiter": [
      "assets/products/gaming-pc/pcap-jupiter/pcap-jupiter.jpg",
      "assets/products/gaming-pc/pcap-jupiter/pcap-jupiter.jpg",
    ],
    "asus-strix-ultra": [
      "assets/products/gaming-pc/asus-strix-ultra/asus-strix-ultra.jpg",
      "assets/products/gaming-pc/asus-strix-ultra/asus-strix-ultra.jpg",
    ],
  },
};
